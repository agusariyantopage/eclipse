...
<?php
  if(empty($_GET['filter'])){
    $sql="SELECT beli.*,pemasok.nama as napem,user.nama as petugas from beli,pemasok,user where beli.id_pemasok=pemasok.id_pemasok and beli.id_user=user.id_user and tanggal_transaksi=DATE(NOW()) and jenis_transaksi='BELI' order by beli.id_beli desc";
  } else {
    if($_GET['filter']=='hari'){      
      $sql="SELECT beli.*,pemasok.nama as napem,user.nama as petugas from beli,pemasok,user where beli.id_pemasok=pemasok.id_pemasok and beli.id_user=user.id_user and tanggal_transaksi=DATE(NOW()) and jenis_transaksi='BELI' order by beli.id_beli desc";
    }
    if($_GET['filter']=='bulan'){      
      $sql="SELECT beli.*,pemasok.nama as napem,user.nama as petugas from beli,pemasok,user where beli.id_pemasok=pemasok.id_pemasok and beli.id_user=user.id_user and MONTH(tanggal_transaksi)=MONTH(NOW()) and jenis_transaksi='BELI' order by beli.id_beli desc";
    }
    if($_GET['filter']=='tahun'){      
      $sql="SELECT beli.*,pemasok.nama as napem,user.nama as petugas from beli,pemasok,user where beli.id_pemasok=pemasok.id_pemasok and beli.id_user=user.id_user and YEAR(tanggal_transaksi)=YEAR(NOW()) and jenis_transaksi='BELI' order by beli.id_beli desc";
    }
    if($_GET['filter']=='semua'){      
      $sql="SELECT beli.*,pemasok.nama as napem,user.nama as petugas from beli,pemasok,user where beli.id_pemasok=pemasok.id_pemasok and beli.id_user=user.id_user and jenis_transaksi='BELI' order by beli.id_beli desc";
    }
  }
